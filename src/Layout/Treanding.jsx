...
const Trending = () => {
    ...
    const productsView = petProducts.slice(0,12)
 ...
 <div className='grid md:grid-cols-4 sm:grid-cols-2 grid-col-1 gap-10 items-center  justify-center md:p-10 sm:p-5 p-0 '>

  {
   productsView.map(product => (
    <Link to={`/equipmentdetails/${product.serviceId}`}> <div key={product.serviceId} className="card my-5 border-2 bg-white h-80 w-72 shadow-md hover:scale-110 transition ease-in-out">
       <figure className=''>
       ...
       <div className="card-body">
         ...
         <div className="card-actions justify-between">
           ...
           <div className="badge badge-outline  text-yellow-600 bg-yellow-100"><img className='h-[15px]' src={ratStar} alt="" />{product.rating}</div>
         </div>
       </div>
     </div></Link>
   ))
